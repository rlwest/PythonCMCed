...
def counter_paper_agent1(memories):
    memories['environment_memory']['agent1hand']['hand'] = 'open'
...
# Retrieval function for Agent 2

def recall_agent2_move(memories):
    memories['environment_memory']['agent2hand'].update({'hand': 'open', 'move': 'rock'})
    print()
    print('[AGENT 2] I play rock!!!!!!!!')

    # decay_all_memory_chunks(memories, 'agent2_declarative_memory', 1)
    # declarative_memory2 = memories['agent2_declarative_memory']
    # lag2 = memories['agent2_working_memory']['lag_buffer']['lag1']
    # lag1 = memories['agent2_working_memory']['lag_buffer']['lag0']
    # matches = {'lag2': lag2, 'lag1': lag1, 'lag0': '*'}  # already shifted lag1
    # negations = {}
    # retrieved_chunk = retrieve_memory_chunk(declarative_memory2, matches, negations)
...
# Agent 2 counters 'paper' by choosing 'scissors' and sets completion flag
...
# Agent 2 counters 'scissors' by choosing 'rock' and sets completion flag
...
# Agent 2 counters 'rock' by choosing 'paper' and sets completion flag
...
def check_agent1_move(memories):
    opponent_move = memories['environment_memory']['agent1hand']['choice']

    print(f"Agent 2 sees that Agent 1 played: {opponent_move}")

    memories['agent2_working_memory']['lag_buffer']['lag0'] = opponent_move

    print(f"Current lag buffer for Agent 2: {memories['agent2_working_memory']['lag_buffer']}")

    lagchunk = memories['agent2_working_memory']['lag_buffer']

    chunk_values = list(lagchunk.values())

    chunk_description = chunk_values

    print("Extracted chunk description:", chunk_description)

    utility_change_by_description(memories, 'agent2_declarative_memory', chunk_description, amount=2, max_utility=200)
...
Agent2Productions.append({
    ...
    'action': check_agent1_move,

    'report': "check_agent1_move"
